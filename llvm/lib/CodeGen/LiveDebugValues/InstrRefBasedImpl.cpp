...
public:
  ...
  /// CreateEmptyPHI - Create a PHI record happening in the given block.
  /// SSAUpdater will populate it with information about incoming values.
  static uint64_t CreateEmptyPHI(LDVSSABlock *BB, unsigned NumPreds,
                                 LDVSSAUpdater *Updater) {
    uint64_t PHIValNum =
        ValueIDNum(BB->BB.getNumber(), 0, Updater->Loc).asU64();
