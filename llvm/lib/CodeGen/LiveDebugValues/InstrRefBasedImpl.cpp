...
void InstrRefBasedLDV::buildVLocValueMap(
  ...
  while (!Worklist.empty() || !Pending.empty()) {
    while (!Worklist.empty()) {
      ...
      // into JoinedInLocs.
      bool InLocsChanged;
      InLocsChanged = vlocJoin(*MBB, LiveOutIdx, LiveInIdx, CurBB,
      ...
      // earlier as we rely on block live-outs.
      ...
      *LiveOutIdx[MBB] = JoinedInLocs;
      ...
      for (auto s : MBB->successors()) {
        ...
        if (BBToOrder[s] > BBToOrder[MBB]) {
          if (OnWorklist.insert(s).second)
            Worklist.push(BBToOrder[s]);
        } else if (OnPending.insert(s).second && OLChanged) {
          Pending.push(BBToOrder[s]);
        }
      }
    }
...
bool InstrRefBasedLDV::ExtendRanges(MachineFunction &MF,
  ...
  if ((unsigned)MaxNumBlocks > InputBBLimit &&
    ...
    for (auto &P : ScopeToVars) {
      buildVLocValueMap(P.first, ScopeToDILocation[P.first], P.second,
