...
unsigned NumSubRegs;
  ...
  MLocTracker(MachineFunction &MF, const TargetInstrInfo &TII,
    ...
    if (SP) {
      unsigned ID = getLocID(SP, false, 0);
  ...
  unsigned getLocID(Register RegOrSpill, bool isSpill, unsigned SpillSubReg) {
    if (!isSpill)
      return RegOrSpill.id();

    unsigned SlotNo = RegOrSpill.id() - 1;
    SlotNo *= NumSubRegs;
    SlotNo += SpillSubReg;
    SlotNo += NumRegs;
    return SlotNo;
  ...
  void defReg(Register R, unsigned BB, unsigned Inst) {
    unsigned ID = getLocID(R, false, 0);
  ...
  LocIdx getOrTrackSpillLoc(SpillLoc L, unsigned ReqdSubReg) {
    ...
    if (SpillID == 0) {
      ...
      for (unsigned SubReg = 0; SubReg < NumSubRegs; ++SubReg) {
        unsigned L = getLocID(SpillID, true, SubReg);
  ...
  Optional<ValueIDNum> readSpill(SpillLoc L, unsigned SubReg) {
    ...
    unsigned LocID = getLocID(SpillID, true, SubReg);
  ...
  Optional<LocIdx> getSpillMLoc(SpillLoc L, unsigned SubReg) {
    ...
    if (SpillID == 0)
      return None;
    unsigned LocNo = getLocID(SpillID, true, SubReg);
  ...
  std::string LocIdxToName(LocIdx Idx) const {
    ...
    if (ID >= NumRegs) {
      ...
      unsigned SubReg = ID % NumSubRegs;
      return Twine("slot ").concat(Twine(Slot).concat(Twine(" subreg ").concat(Twine(SubReg)))).str();
    } else {
...
public:
                  ...
                  const DbgValueProperties &Properties) override
  {
    ...
    // Explicitly terminate with a DBG_VALUE any stack writes that clobber
    // varible values. DbgEntitytHistoryCalculator can't work those out.
...
public:
  ...
  TransferTracker(const TargetInstrInfo *TII, MLocTracker *MTracker,
                  ...
                  LocEmitter &Emitter)
      : TII(TII), MTracker(MTracker), MF(MF), Emitter(Emitter), TRI(TRI),
  ...
  void loadInlocs(MachineBasicBlock &MBB, ValueIDNum *MLocs,
    ...
    for (auto Var : VLocs) {
      if (Var.second.Kind == DbgValue::Const) {
        Emitter.emitLoc(Var.second.MO, Var.first, Var.second.Properties);
        continue;
      }
      ...
      Emitter.emitLoc(*M, Var.first, Var.second.Properties);
    }
  }
  ...
  /// After the instruction at index \p Inst has been
  ...
  bool recoverAsEntryValue(const DebugVariable &Var, DbgValueProperties &Prop,
    ...
    Emitter.emitLoc(MO, Var, {NewExpr, Prop.Indirect});
    return true;
  }
  ...
  void clobberMloc(LocIdx MLoc) {
    ...
    for (auto &Var : ActiveMLocIt->second) {
      ...
      // Was there a new location?
      if (NewLoc) {
        Emitter.emitLoc(NewLoc, Var, Properties);
        ActiveVLocIt->second.Loc = *NewLoc;
        NewMLocs.insert(Var);
      } else if (recoverAsEntryValue(Var, Properties, OldValue)) {
        ActiveVLocs.erase(ActiveVLocIt);
      } else {
        Emitter.clobberLoc(MLoc, Var, Properties);
        ActiveVLocs.erase(ActiveVLocIt);
      }
    }
  ...
  void transferMlocs(LocIdx Src, LocIdx Dst) {
    ...
    for (auto &Var : ActiveMLocs[Src]) {
      ...
      assert(Dst != 0);
      Emitter.emitLoc(Dst, Var, ActiveVLocIt->second.Properties);
    }
...
bool InstrRefBasedLDV::transferDebugInstrRef(MachineInstr &MI,
  ...
  auto PHIIt = std::lower_bound(DebugPHINumToValue.begin(), DebugPHINumToValue.end(), InstNo);
  if (InstrIt != DebugInstrNumToInstr.end()) {
    ...
    if (OpNo == MachineFunction::DebugOperandMemNumber &&
      ...
      // XXX XXX XXX -- good assumption that folded spills always write the
      // whole slot? I guess it always writes something large with no offset,
      // so defs everything.
      auto Opt = MTracker->getSpillMLoc(SpillLoc, 0);
      if (Opt) {
        LocIdx L = MTracker->getOrTrackSpillLoc(SpillLoc, 0);
        NewID = ValueIDNum(BlockNo, InstrIt->second.second, L);
      } else {
        // If the slot wasn't tracked, don't try to track it!
        NewID = None;
      }
      ...
      unsigned LocID = MTracker->getLocID(MO.getReg(), false, 0);
  ...
  TTracker->Emitter.emitLoc(FoundLoc, V, Properties);
  return true;
}
...
bool InstrRefBasedLDV::transferDebugPHI(MachineInstr &MI) {
  ...
  if (MO.isReg()) {
    ...
    // to express that if it happens. In the mean time, the PHI always refers
    // to the whole slot, with no subreg.
    unsigned SubReg = 0;
...
void InstrRefBasedLDV::transferRegisterDef(MachineInstr &MI) {
      ...
      !(*MI.memoperands_begin())->getPseudoValue()->isAliased(MFI))
 {
    ...
    for (unsigned SubReg = 0; SubReg < MTracker->NumSubRegs; ++SubReg) {
      LocIdx L = MTracker->getOrTrackSpillLoc(Spill, SubReg);
      MTracker->setSpill(Spill, SubReg, ValueIDNum(CurBB, CurInst, L));
    }
  }
...
for (unsigned SubReg = 0; SubReg < MTracker->NumSubRegs; ++SubReg) {
  ValueIDNum clobbervalue = {CurBB, CurInst, *MTracker->getSpillMLoc(*Loc, SubReg)};
  MTracker->setSpill(*Loc, SubReg, clobbervalue);
}

// OK, transfer subreg bits.
for (MCSubRegIterator foo(Reg, TRI, true); foo.isValid(); ++foo) {
  unsigned subreg = TRI->getSubRegIndex(lolreg, *foo);
  ...
  if (TTracker)
    TTracker->transferMlocs(SrcIdx, DstLoc);
...
// Assumption: we're reading from the base of the stack slot, not some offset
...
for (unsigned SubReg = 0; SubReg < MTracker->NumSubRegs; ++SubReg) {
      ...
      LocIdx L = MTracker->getOrTrackSpillLoc(*Loc, SubReg);
      ...
      MTracker->setSpill(*Loc, SubReg, ValueID);
}
    }
  }
  return true;
...
// XXX for assertions,
ValueIDNum NewID = MTracker->getNumAtPos(Location.Idx);
ValueIDNum NewID2 = MTracker->getNumAtPos(NewID.getLoc());
(void)NewID2;
...
bool InstrRefBasedLDV::ExtendRangesImpl(MachineFunction &MF,
  ...
  for (auto It = EarlyInc.begin(); It != EarlyInc.end(); ++It) {
    ...
    auto &InstAndNum = *It;
    ValueIDNum &Num = InstAndNum.ValueRead;
    if (!Num.isPHI())
      continue;
...
Optional<ValueIDNum> InstrRefBasedLDV::resolveDbgPHIs(MachineFunction &MF,
  ...
  auto RangePair = std::equal_range(DebugPHINumToValue.begin(), DebugPHINumToValue.end(), InstrNum);
