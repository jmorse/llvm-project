...
public:
  ...
  IndexedMap<ValueIDNum, LocIdxToIndexFunctor> LocIdxToIDNum;
  ...
  MLocTracker(MachineFunction &MF, const TargetInstrInfo &TII,
    ...
    reset();
    LocIdxToIDNum.grow(LocIdx(0));
    LocIdxToIDNum[LocIdx(0)] = ValueIDNum::EmptyValue;
    LocIDToLocIdx.resize(NumRegs, LocIdx(0));
    LocIDToLocIdx[0] = LocIdx(0);
    LocIdxToLocID.grow(0);
  ...
  void setMPhis(unsigned NewCurBB) {
    ...
    for (unsigned ID = 1; ID < LocIdxToIDNum.size(); ++ID) {
      LocIdxToIDNum[LocIdx(ID)] = {CurBB, 0, ID};
    }
  }
  ...
  void reset(void) {
    for (unsigned ID = 1; ID < LocIdxToIDNum.size(); ++ID)
      LocIdxToIDNum[LocIdx(ID)] = ValueIDNum::EmptyValue;
    Masks.clear();
  }
  ...
  void writeRegMask(const MachineOperand *MO, unsigned CurBB, unsigned InstID) {
    ...
    for (unsigned int I = 1; I < LocIdxToIDNum.size(); ++I) {
      unsigned ID = LocIdxToLocID[LocIdx(I)];
  ...
  void dump() const {
    for (unsigned int ID = 0; ID < LocIdxToIDNum.size(); ++ID) {
      auto &ValueID = LocIdxToIDNum[LocIdx(ID)];
      if (ValueID.getLoc() == 0)
        continue;
      std::string MLocName = LocIdxToName(ValueID.getLoc());
...
public:
  ...
  typedef enum {
    ...
    Proposed,  // This is a tentative PHI value, which may be confirmed or
               // invalidated later.
    NoVal      // Empty DbgValue. ID.BlockNo stores which block this was
               // generated in.
   } KindT;
  ...
  bool operator==(const DbgValue &Other) const {
    if (std::tie(Kind, Properties) != std::tie(Other.Kind, Other.Properties))
      ...
      return false;
    else if (Kind == NoVal && ID != Other.ID)
      return false;
...
public:
  ...
  void defVar(const MachineInstr &MI, ValueIDNum ID) {
    ...
    DbgValueProperties Properties(
        {MI.getDebugExpression(), MI.getOperand(1).isImm()});
    DbgValue Rec = {{ID}, Properties, DbgValue::Def};
  ...
  void defVar(const MachineInstr &MI, const MachineOperand &MO) {
    ...
    DebugVariable Var(MI.getDebugVariable(), MI.getDebugExpression(),
                      MI.getDebugLoc()->getInlinedAt());
    DbgValueProperties Properties(MI.getDebugExpression(),
                                  MI.getOperand(1).isImm());
    ValueIDNum EmptyValue = ValueIDNum::EmptyValue;
    DbgValue Rec = {{EmptyValue}, Properties, DbgValue::Const};
    Rec.MO = MO;
...
public:
  ...
  void loadInlocs(MachineBasicBlock &MBB, uint64_t *MLocs,
    ...
    ActiveVLocs.clear();
    VarLocs.clear();
    VarLocs.resize(NumLocs, ValueIDNum::EmptyValue);
    ...
    for (unsigned Idx = 1; Idx < NumLocs; ++Idx) {
      ...
      if (it == ValueToLoc.end() || MTracker->isSpill(it->second) ||
          (!isCalleeSaved(it->second) && isCalleeSaved(Idx))) {
        // Insert, or overwrite if insertion failed.
        auto PrefLocRes = ValueToLoc.insert(std::make_pair(VNum, LocIdx(Idx)));
        if (!PrefLocRes.second)
          PrefLocRes.first->second = LocIdx(Idx);
      }
    }
  ...
  void clobberMloc(LocIdx MLoc, MachineBasicBlock::iterator Pos) {
    ...
    for (auto &Var : ActiveMLocIt->second) {
      ...
      PendingDbgValues.push_back(MTracker->emitLoc(Idx, Var, Properties));
...
private:
  ...
  pickVPHILoc(MachineBasicBlock &MBB, const DebugVariable &Var,
              const LiveIdxT &LiveOuts, uint64_t **MOutLocs, uint64_t **MInLocs,
...
ValueIDNum ValueIDNum::EmptyValue = {0, 0, 0};
...
bool InstrRefBasedLDV::transferDebugValue(const MachineInstr &MI) {
  ...
  if (VTracker) {
    if (MO.isReg()) {
      ...
      ValueIDNum Undef = ValueIDNum::EmptyValue;
...
void InstrRefBasedLDV::produceTransferFunctions(
  ...
  for (unsigned int I = 1; I < MTracker->LocIdxToIDNum.size(); ++I) {
    unsigned ID = MTracker->LocIdxToLocID[LocIdx(I)];
  ...
  for (unsigned int I = 0; I < MaxNumBlocks; ++I) {
    ...
    for (unsigned Bit : BV.set_bits()) {
      ...
      // Attempt to install a "no-value" assignment to erase any value number.
      // If there's already one set, and it's a PHI, then clobber it.
      auto Result =
        TransferMap.insert(std::make_pair(Idx.asU64(), ValueIDNum::EmptyValue));
      if (!Result.second) {
        ValueIDNum &ValueID = Result.first->second;
        if (ValueID.getBlock() == I && ValueID.isPHI())
          // it was left as live-through. Set it to clobbered.
          ValueID = ValueIDNum::EmptyValue;
      }
    }
  }
}
...
InstrRefBasedLDV::mlocJoin(MachineBasicBlock &MBB,
  ...
  for (unsigned Idx = 1; Idx < MTracker->getNumLocs(); ++Idx) {
    ...
    if (Disagree && !NonBackEdgeDisagree &&
        ValueIDNum::fromU64(InLocs[Idx]).getLoc() != 0) {
      ...
      unsigned BaseBlockRPONum = BBNumToRPO[BaseID.getBlock()] + 1;
      if (!BaseID.isPHI())
        BaseBlockRPONum = 0;
  ...
  while (!Worklist.empty() || !Pending.empty()) {
    ...
    while (!Worklist.empty()) {
      ...
      for (auto &P : MLocTransfer[CurBB]) {
        ValueIDNum NewID = ValueIDNum::EmptyValue;
        if (P.second.getBlock() == CurBB && P.second.isPHI()) {
          // This is a movement of whatever was live in. Read it.
          NewID = MTracker->getNumAtPos(P.second.getLoc());
        } else {
          // It's a def. Just set it.
          assert(P.second.getBlock() == CurBB || P.second.getLoc() == 0);
          NewID = P.second;
        }
...
bool InstrRefBasedLDV::vlocDowngradeLattice(
  ...
  if (OldLiveInLocation.Kind == DbgValue::NoVal) {
    ...
    if (OldLiveInLocation.ID.getBlock() == (unsigned)MBB.getNumber())
      return false;
...
std::tuple<Optional<ValueIDNum>, bool> InstrRefBasedLDV::pickVPHILoc(
  ...
  for (auto p : BlockOrders) {
    ...
    ValueIDNum ValToLookFor = {0, 0, LocIdx(0)};

    if (OutVal.Kind == DbgValue::Const || OutVal.Kind == DbgValue::NoVal) {
      // Consts and no-values cannot have locations we can join on.
      continue;
    } else if (OutVal.Kind == DbgValue::Proposed) {
      // Treat proposed values as any other value, and seek a location.
      ValToLookFor = OutVal.ID;
    } else {
    ...
    for (unsigned int I = 0; I < NumLocs; ++I) {
      if (MOutLocs[ThisBBNum][I] == ValToLookFor.asU64())
...
std::tuple<bool, bool> InstrRefBasedLDV::vlocJoin(
    ...
    const SmallSet<DebugVariable, 4> &AllVars, uint64_t **MOutLocs,
    uint64_t **MInLocs,
    SmallPtrSet<const MachineBasicBlock *, 8> &InScopeBlocks,
    SmallPtrSet<const MachineBasicBlock *, 8> &BlocksToExplore,
    DenseMap<DebugVariable, DbgValue> &InLocsT) {
  ...
  auto ConfirmEmptyValue = [&ConfirmValue, &MBB](const DebugVariable &Var, const DbgValueProperties &Properties) {
    unsigned BlockNo = MBB.getNumber();
    ValueIDNum tmp = {BlockNo, 0, LocIdx(0)};
    DbgValue NoLocPHIVal = {{tmp}, Properties, DbgValue::NoVal};
  ...
  for (auto &Var : AllVars) {
    ...
    if (OurState == Agreed) {
      ...
      if (FirstID.getBlock() == (uint64_t)MBB.getNumber() && FirstID.isPHI()) {
        ConfirmValue(Var, {{FirstID}, Properties, DbgValue::Def});
        ...
        ConfirmValue(Var, {{FirstID}, Properties, DbgValue::Proposed});
      }
      ...
      if (VPHI && AllEdgesVPHI) {
        ...
        ConfirmValue(Var, {{*VPHI}, Properties, K});
      } else if (VPHI) {
        ...
        DbgValue NoBEValue = {{*VPHI}, Properties, DbgValue::Proposed};
        ConfirmValue(Var, NoBEValue);
      } else {
        ConfirmEmptyValue(Var, Properties);
      }
    } else {
      ...
      if (FirstVisit) {
        for (auto &Transfer : VTracker.Vars) {
          if (Transfer.second.Kind == DbgValue::Def &&
            ...
            Transfer.second.ID = ValueIDNum::fromU64(MInLocs[CurBB][Loc.asU64()]);
          }
        }
      }
