...
Optional<ValueIDNum> InstrRefBasedLDV::pickVPHILoc(
  ...
  for (auto p : BlockOrders) {
    ...
    if (OutValIt == LiveOuts.end())
      return None; // No predecessor -> we're stuffed 
...
bool InstrRefBasedLDV::vlocJoin(
  ...
  for (auto p : BlockOrders) {
    ...
    // Live-outs must have been initialized.
...
for (auto &Var : VarsWeCareAbout) {
  ...
  while (!Worklist.empty() || !Pending.empty()) {
    while (!Worklist.empty()) {
      ...
      if (LiveIn->Kind == DbgValue::VPHI && LiveIn->BlockNo == CurBB) {
  ...
  for (auto *MBB : BlockOrders) {
    ...
    if (BlockLiveIn->Kind == DbgValue::NoVal)
      continue;
    if (BlockLiveIn->Kind == DbgValue::VPHI && BlockLiveIn->ID == ValueIDNum::EmptyValue)
      continue;
