...
public:
  ...
  DbgValue(const ValueIDNum &Val, const DbgValueProperties &Prop, KindT Kind)
      : ID(Val), BlockNo(0), MO(None), Properties(Prop), Kind(Kind) {
    assert(Kind == Def);
  }

  DbgValue(unsigned BlockNo, const DbgValueProperties &Prop, KindT Kind)
      : ID(ValueIDNum::EmptyValue), BlockNo(BlockNo), MO(None), Properties(Prop), Kind(Kind) {
    assert(Kind == NoVal || Kind == VPHI);
  }

  DbgValue(const MachineOperand &MO, const DbgValueProperties &Prop, KindT Kind)
      : ID(ValueIDNum::EmptyValue), BlockNo(0), MO(MO), Properties(Prop), Kind(Kind) {
    assert(Kind == Const);
  }

  DbgValue(const DbgValueProperties &Prop, KindT Kind)
    : ID(ValueIDNum::EmptyValue), BlockNo(0), MO(None), Properties(Prop), Kind(Kind) {
    assert(Kind == Undef &&
...
private:
  ...
  /// through.
  void buildVLocValueMap(const LexicalScope *Scope, const DILocation *DILoc,
  ...
  /// stored into \p VLOCInLocs. \returns true if the live-ins are modified.
