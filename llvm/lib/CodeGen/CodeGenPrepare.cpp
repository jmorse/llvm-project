...
bool CodeGenPrepare::isMergingEmptyBlockProfitable(BasicBlock *BB,
  ...
  if (BB->getTerminator() != &*BB->getFirstNonPHIIt())
    return true;
...
bool CodeGenPrepare::optimizeCallInst(CallInst *CI, ModifyDT &ModifiedDT) {
  ...
  if (II) {
    ...
    case Intrinsic::aarch64_stxr: {
      ...
      ExtVal->moveBefore(CI->getIterator()); // XXX move deliberately to call position
  ...
  if (Changed && !BB->hasAddressTaken() && pred_empty(BB)) {
    ...
    for (auto *CI : CallInsts) {
      for (auto const *FakeUse : FakeUses) {
        ...
        ClonedInst->insertBefore(CI->getIterator()); // inserting at call-sites, is meaning, not position
      }
    }
    BB->eraseFromParent();
  }
...
class TypePromotionTransaction {
  ...
  class InsertionHandler {
    ...
    struct {
// XXX was a union, turn into a variant?
...
bool CodeGenPrepare::optimizeSelectInst(SelectInst *SI) {
  ...
  for (Instruction *I : TrueInstrs)
    I->moveBefore(TrueBranch->getIterator()); // deliberately moving insts to these branch positions
...
bool CodeGenPrepare::tryToSinkFreeOperands(Instruction *I) {
  ...
  for (Use *U : ToReplace) {
    ...
    NI->insertBefore(InsertPoint->getIterator()); // Operands are being sunk to their users; it's based on how things are used rather than their abstract positions. There should be no debug-info concerns.
    InsertPoint = NI;
...
static void DbgInserterHelper(DbgVariableRecord *DVR, BasicBlock::iterator VI) {
  ...
  if (isa<PHINode>(VI))
    ...
    VIBB->insertDbgRecordAfter(DVR, &*VI); // XXX sketchy
}
...
bool CodeGenPrepare::splitBranchCondition(Function &F, ModifyDT &ModifiedDT) {
  ...
  for (auto &BB : F) {
    ...
    if (auto *I = dyn_cast<Instruction>(Cond2)) {
      I->removeFromParent();
      I->insertBefore(Br2->getIterator()); // Insert before just created br, no debug-info concerns
    }
