...
DPMarker *BasicBlock::createMarker(Instruction *I) {
  assert(IsNewDbgInfoFormat && "Tried to create a marker in a non new debug-info block!");
  if (I->DbgMarker)
    return I->DbgMarker;
...
bool BasicBlock::validateDbgValues(bool Assert, bool Msg) {
  bool RetVal = false;

  // Helper lambda for reporting failures: via assertion, printing, and return
  // value.
  auto TestFailure = [Assert, Msg, &RetVal](bool Val, const char *Text) {
    // Did the test fail?
    if (Val)
      return;
  ...
  TestFailure(TrailingDPValues.StoredDPValues.empty(), "Trailing DPValues in block");
  return RetVal;
}
...
void BasicBlock::spliceDebugInfoEmptyBlock(BasicBlock::iterator Dest,
  ...
  if (Src->empty()) {
    ...
    if (Src->TrailingDPValues.StoredDPValues.empty())
      return;
...
void BasicBlock::spliceDebugInfo(BasicBlock::iterator Dest, BasicBlock *Src,
  ...
  /*
    ...
    maintenence. However, we have to do special things with the DPValues marked
    with the +:= characters. We only have three positions: should the "+"
...
void BasicBlock::insertDPValueAfter(DPValue *DPV, Instruction *I) {
  ...
  DPMarker *NextMarker =
      (NextIt == end()) ? &TrailingDPValues : NextIt->DbgMarker;
...
void BasicBlock::undoInstrRemoval(Instruction *I, DPValue::self_iterator Pos) {
  ...
  if (Pos == TrailingDPValues.StoredDPValues.end()) {
