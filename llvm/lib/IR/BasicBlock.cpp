...
bool BasicBlock::validateDbgValues(bool Assert, bool Msg, raw_ostream *OS) {
  ...
  // value.
  auto TestFailure = [this, Assert, Msg, &RetVal, OS](bool Val, const char *Text) {
    // Did the test fail?
    if (Val)
      return;
...
assert(!getTrailingDPValues() && "Destroyed block had trailers?");
}
...
void BasicBlock::spliceDebugInfoEmptyBlock(BasicBlock::iterator Dest,
  ...
  if (!Src->empty() && First == Src->begin() && ReadFromHead && First->hasDbgValues()) {
    createMarker(Dest);
...
//    createMarker(&*Dest);
  ...
  if (ReadFromTail && Src->getMarker(Last)) {
    if (Last != Src->end()) { //XXX jmorse utilityify
      if (!Dest->adoptDbgValues(Src, Last, true) && LastIsEnd)
        Src->getMarker(Last)->eraseFromParent();
    } else {
