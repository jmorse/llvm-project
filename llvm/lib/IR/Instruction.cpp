...
BasicBlock::DIIterator Instruction::cloneDebugInfoFrom(const Instruction *From, bool InsertAtHead, std::optional<BasicBlock::DIIterator> FromHere) {
  if (!From->DbgMarker)
    return Parent->LolDbgValuesOffEnd.StoredDPValues.end();
  ...
  return DbgMarker->cloneDbgInfoFrom(From->DbgMarker, InsertAtHead, FromHere);
}
...
auto Instruction::getDbgValueRange() const -> iterator_range<BasicBlock::DIIterator> {
  ...
  if (!DbgMarker) {
    auto It = Parent->LolDbgValuesOffEnd.StoredDPValues.end();
