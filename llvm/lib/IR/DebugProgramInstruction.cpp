...
BasicBlock::DIIterator DPMarker::getFirstDbgValue() {
  assert(MarkedInstr);
  return StoredDPValues.begin();
}
...
simple_ilist<DPValue>::iterator DPMarker::cloneDbgInfoFrom(DPMarker *From, bool InsertAtHead, std::optional<simple_ilist<DPValue>::iterator> from_here) {
  ...
  if (InsertAtHead) {
    for (DPValue &DPV : reverse(Range)) {
      ...
      New->setMarker(this);
      StoredDPValues.insert(StoredDPValues.begin(), *New);
      if (!First)
        First = New;
    }
  } else {
  ...
  if (!First)
    return StoredDPValues.end();

  return First->getIterator();
}

iterator_range<simple_ilist<DPValue>::iterator> DPMarker::getRangeToEnd(simple_ilist<DPValue>::iterator it) {
  return make_range(it, StoredDPValues.end());
}
