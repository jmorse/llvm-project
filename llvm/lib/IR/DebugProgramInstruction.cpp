...
void DPMarker::removeMarker() {
  ...
  } else if (NextMarker == &Owner->getParent()->TrailingDPValues) {
    NextMarker->absorbDebugValues(*this, true);
  } else {
...
iterator_range<simple_ilist<DPValue>::iterator> DPMarker::cloneDebugInfoFrom(
  ...
  if (InsertAtHead)
    // If InsertAtHead is set, we clone in reverse order. The range of newly
    // cloned DPValues is from begin() to the first cloned, plus one.
    return {StoredDPValues.begin(), std::next(First->getIterator())};
  else
