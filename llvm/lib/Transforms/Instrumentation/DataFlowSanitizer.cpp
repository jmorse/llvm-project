...
class DataFlowSanitizer {
  ...
  Value *getShadowAddress(Value *Addr, BasicBlock::iterator Pos, Value *ShadowOffset);
  std::pair<Value *, Value *>
  getShadowOriginAddress(Value *Addr, Align InstAlignment, BasicBlock::iterator Pos);
...
struct DFSanFunction {
  ...
  Value *combineOrigins(const std::vector<Value *> &Shadows,
                        const std::vector<Value *> &Origins, BasicBlock::iterator Pos,
...
private:
  ...
  ///   instrumentations.
  void storeOrigin(BasicBlock::iterator Pos, Value *Addr, uint64_t Size, Value *Shadow,
  ...
  loadShadowOriginSansLoadTracking(Value *Addr, uint64_t Size,
                                   Align InstAlignment, BasicBlock::iterator Pos);
...
Value *DataFlowSanitizer::getShadowAddress(Value *Addr, BasicBlock::iterator Pos,
                                           Value *ShadowOffset) {
  IRBuilder<> IRB(Pos->getParent(), Pos);
...
// Generates IR to compute the union of the two given shadows, inserting it
// before Pos. The combined value is with primitive type.
Value *DFSanFunction::combineShadows(Value *V1, Value *V2, BasicBlock::iterator Pos) {
...
Value *DFSanFunction::combineOperandShadows(Instruction *Inst) {
  ...
  for (unsigned I = 1, N = Inst->getNumOperands(); I < N; ++I)
    Shadow = combineShadows(Shadow, getShadow(Inst->getOperand(I)), Inst->getIterator());

  return expandFromPrimitiveShadow(Inst->getType(), Shadow, Inst->getIterator());
}
...
Value *DFSanFunction::combineOrigins(const std::vector<Value *> &Shadows,
                                     ...
                                     BasicBlock::iterator Pos, ConstantInt *Zero) {
...
Value *DataFlowSanitizer::loadNextOrigin(BasicBlock::iterator Pos, Align OriginAlign,
                                         Value **OriginAddr) {
  IRBuilder<> IRB(Pos->getParent(), Pos);
  *OriginAddr =
...
void DFSanFunction::storeOrigin(BasicBlock::iterator Pos, Value *Addr, uint64_t Size,
...
void DFSanVisitor::visitSelectInst(SelectInst &I) {
  ...
  if (isa<VectorType>(I.getCondition()->getType())) {
    ...
    if (TrueShadow == FalseShadow) {
      ...
      ShadowSel =
          SelectInst::Create(I.getCondition(), TrueShadow, FalseShadow, "", I.getIterator());
      if (ShouldTrackOrigins) {
  ...
  DFSF.setShadow(&I, ClTrackSelectControlFlow
                         ? DFSF.combineShadowsThenConvert(
                               I.getType(), CondShadow, ShadowSel, I.getIterator())
                         : ShadowSel);
  if (ShouldTrackOrigins) {
...
void DFSanVisitor::addShadowArguments(Function &F, CallBase &CB,
  ...
  for (unsigned N = FT->getNumParams(); N != 0; ++I, --N)
    Args.push_back(DFSF.collapseToPrimitiveShadow(DFSF.getShadow(*I), CB.getIterator()));
  ...
  if (FT->isVarArg()) {
    ...
    for (unsigned N = 0; I != CB.arg_end(); ++I, ++N) {
      ...
      IRB.CreateStore(DFSF.collapseToPrimitiveShadow(DFSF.getShadow(*I), CB.getIterator()),
                      LabelVAPtr);
    }
...
void DFSanVisitor::visitPHINode(PHINode &PN) {
  ...
  PHINode *ShadowPN =
      PHINode::Create(ShadowTy, PN.getNumIncomingValues(), "", PN.getIterator());
  ...
  if (DFSF.DFS.shouldTrackOrigins()) {
    OriginPN =
        PHINode::Create(DFSF.DFS.OriginTy, PN.getNumIncomingValues(), "", PN.getIterator());
