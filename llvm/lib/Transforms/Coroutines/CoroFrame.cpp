...
static void insertSpills(const FrameDataInfo &FrameData, coro::Shape &Shape) {
  ...
  for (auto const &E : FrameData.Spills) {
    ...
    if (auto *Arg = dyn_cast<Argument>(Def)) {
      ...
      InsertPt = CSI->getParent()->getSingleSuccessor()->getFirstInsertionPt();
    } else {
    ...
    for (auto *U : E.second) {
      ...
      if (CurrentBlock != U->getParent()) {
        ...
        Builder.SetInsertPoint(CurrentBlock, CurrentBlock->getFirstInsertionPt());
  ...
  Builder.SetInsertPoint(Shape.AllocaSpillBlock, Shape.AllocaSpillBlock->begin());
  ...
  for (const auto &A : FrameData.Allocas) {
    ...
    SmallVector<DPValue *, 4> DPVs;
    findDbgUsers(DIs, DPVs, Alloca);
    for (auto *DVI : DIs)
...
static void sinkSpillUsesAfterCoroBegin(Function &F,
  ...
  for (Instruction *Inst : InsertionList)
    Inst->moveBeforeBreaking(InsertPt);
}
...
void coro::salvageDebugInfo(
  ...
  if (!isa<DbgValueInst>(DVI) && !isa<DbgAddrIntrinsic>(DVI)) {
    // jmorse: these are all Breaking because they're deliberately moving a
    // dbg.declare, which is not a dbg.value. A future implemention will
    // probably do dbg.declares differently.
    if (auto *II = dyn_cast<InvokeInst>(Storage))
      DVI->moveBeforeBreaking(II->getNormalDest()->getFirstNonPHI());
    else if (auto *CBI = dyn_cast<CallBrInst>(Storage))
      DVI->moveBeforeBreaking(CBI->getDefaultDest()->getFirstNonPHI());
      ...
      DVI->moveAfterBreaking(InsertPt);
    } else if (isa<Argument>(Storage))
      DVI->moveAfterBreaking(F->getEntryBlock().getFirstNonPHI());
  }
}
...
void coro::buildCoroutineFrame(Function &F, Shape &Shape) {
  ...
  for (auto &Iter : FrameData.Spills) {
    ...
    findDbgValues(DVIs, DPVs, V);
    for (DbgValueInst *DVI : DVIs)
      if (Checker.isDefinitionAcrossSuspend(*V, DVI))
        FrameData.Spills[V].push_back(DVI);
    // TODO: Handle DPValues correctly.
  }
