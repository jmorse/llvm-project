...
bool ObjCARCOpt::OptimizeInlinedAutoreleaseRVCall(
  ...
  CallInst *Release = CallInst::Create(
      EP.get(ARCRuntimeEntryPointKind::Release), CallArg, "", Inst->getIterator());
...
void ObjCARCOpt::OptimizeIndividualCallImpl(Function &F, Instruction *Inst,
  ...
  if (IsAutorelease(Class) && Inst->use_empty()) {
    ...
    if (Arg) {
      ...
      CallInst *NewCall =
          CallInst::Create(Decl, Call->getArgOperand(0), "", Call->getIterator());
  ...
  do {
    ...
    for (unsigned i = 0, e = PN->getNumIncomingValues(); i != e; ++i) {
      ...
      BasicBlock::iterator InsertPos = PN->getIncomingBlock(i)->back().getIterator();
...
void ObjCARCOpt::MoveCalls(Value *Arg, RRInfo &RetainsToMove,
  ...
  for (Instruction *InsertPt : ReleasesToMove.ReverseInsertPts) {
    Value *MyArg = ArgTy == ParamTy ? Arg :
                   new BitCastInst(Arg, ParamTy, "", InsertPt->getIterator());
    ...
    CallInst *Call = CallInst::Create(Decl, MyArg, BundleList, "", InsertPt->getIterator());
      ...
      case ARCInstKind::LoadWeakRetained: {
        ...
        case AliasResult::MustAlias:
          ...
          if (Class == ARCInstKind::LoadWeakRetained) {
            Function *Decl = EP.get(ARCRuntimeEntryPointKind::Retain);
            CallInst *CI = CallInst::Create(Decl, EarlierCall, "", Call->getIterator());
            CI->setTailCall();
          }
