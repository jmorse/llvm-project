...
void ObjCARCContract::tryToContractReleaseIntoStoreStrong(
  ...
  CallInst *StoreStrong =
      objcarc::createCallInstWithColors(Decl, Args, "", Store->getIterator(), BlockColors);
...
bool ObjCARCContract::tryToPeepholeInstruction(
  ...
  case ARCInstKind::UnsafeClaimRV: {
    ...
    if (GetRCIdentityRoot(&*BBI) == GetArgRCIdentityRoot(Inst)) {
      ...
      objcarc::createCallInstWithColors(IA, std::nullopt, "", Inst->getIterator(),
                                        BlockColors);
    }
      ...
      for (Value::use_iterator UI = Arg->use_begin(), UE = Arg->use_end();
        ...
        if (PHINode *PHI = dyn_cast<PHINode>(U.getUser())) {
          ...
          if (Replacement->getType() != UseTy) {
            ...
            Replacement =
                new BitCastInst(Replacement, UseTy, "", InsertBB->back().getIterator());
          }
