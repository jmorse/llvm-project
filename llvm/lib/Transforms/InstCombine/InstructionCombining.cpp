...
bool InstCombinerImpl::freezeOtherUses(FreezeInst &FI) {
  ...
  if (isa<Argument>(Op)) {
    ...
    while (isa<AllocaInst>(MoveBefore))
      MoveBefore = std::next(MoveBefore);
    ...
    MoveBefore = I->getNextNonDebugInstruction()->getIterator();
  }

  bool Changed = false;
  if (&FI != &*MoveBefore) {
    FI.moveBeforeBreaking(*MoveBefore->getParent(), MoveBefore);
    Changed = true;
  }
...
static bool TryToSinkInstruction(Instruction *I, BasicBlock *DestBlock,
  ...
  I->moveBeforeBreaking(*DestBlock, InsertPos);
  ++NumSunkInst;
