...
void SSAUpdaterBulk::RewriteAllUses(DominatorTree *DT,
  ...
  for (auto &R : Rewrites) {
    ...
    DefBlocks.reserve(R.Defines.size()); # warning useless if it's usually one?
    for (auto &Def : R.Defines)
      DefBlocks.insert(Def.first);
    IDF.setDefiningBlocks(DefBlocks);

    SmallPtrSet<BasicBlock *, 2> UsingBlocks;
    UsingBlocks.reserve(R.Uses.size()); # warning useless if it's usually one?
    for (Use *U : R.Uses)
    ...
    ProcessedUses.reserve(R.Uses.size()); #warning might be deduplicating?
    for (Use *U : R.Uses) {
