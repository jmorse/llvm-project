...
bool WidenIV::widenWithVariantUse(WidenIV::NarrowIVDefUse DU) {
  ...
  for (PHINode *User : LCSSAPhiUsers) {
    ...
    Builder.SetInsertPoint(User->getParent(), User->getParent()->getFirstInsertionPt());
  ...
  if (PHINode *UsePhi = dyn_cast<PHINode>(DU.NarrowUse)) {
    if (LI->getLoopFor(UsePhi->getParent()) != L) {
      ...
      if (UsePhi->getNumOperands() != 1)
        ...
        Builder.SetInsertPoint(WidePhi->getParent(), WidePhi->getParent()->getFirstInsertionPt());
