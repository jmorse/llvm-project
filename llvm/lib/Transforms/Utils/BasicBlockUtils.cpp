...
SplitBlockPredecessorsImpl(BasicBlock *BB, ArrayRef<BasicBlock *> Preds,
  ...
  if (LI && LI->isLoopHeader(BB)) {
    ...
    BI->setDebugLoc(BB->getFirstNonPHIIt()->getDebugLoc());
...
static void SplitLandingPadPredecessorsImpl(
  ...
  BI1->setDebugLoc(OrigBB->getFirstNonPHIIt()->getDebugLoc()); // XXX leaked dbg.value location
  ...
  if (!NewBB2Preds.empty()) {
    ...
    BI2->setDebugLoc(OrigBB->getFirstNonPHIIt()->getDebugLoc()); // XXX leaked dbg.value location
