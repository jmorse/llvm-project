...
Value *GuardWideningImpl::freezeAndPush(Value *Orig, Instruction *InsertPt) {
  if (isGuaranteedNotToBePoison(Orig, nullptr, InsertPt, &DT))
    return Orig;
  std::optional<BasicBlock::iterator> InsertPtAtDef = getFreezeInsertPt(Orig, DT);
  if (!InsertPtAtDef) {
  ...
  if (isa<Constant>(Orig) || isa<GlobalValue>(Orig)) {
    FreezeInst *FI = new FreezeInst(Orig, "gw.freeze");
    FI->insertBefore(*(*InsertPtAtDef)->getParent(), *InsertPtAtDef);
    return FI;
  }
  ...
  auto handleConstantOrGlobal = [&](Use &U) {
    ...
    if (Visited.insert(Def).second) {
      ...
      CacheOfFreezes[Def]->insertBefore(*InsertPt->getParent(), InsertPt);
    }
