...
calculateFragment(uint64_t NewStorageSliceOffsetInBits,
  ...
  // If the base storage describes part of the variable apply offset and size
  // constraint.
  if (StorageFragment) {
    Target.SizeInBits =
  ...
  // If the target and existing fragment don't overlap at all then the
  // dbg.assign should not be used to describe this assignment (it may get
  // used for other slices).
  if (CurrentFragment->OffsetInBits + CurrentFragment->SizeInBits <=
          Target.OffsetInBits ||
      CurrentFragment->OffsetInBits >= Target.OffsetInBits + Target.SizeInBits)
    return std::nullopt;

  // We know there's some overlap - now reject overlaps where target is not
  // fully contained within the current fragment. TODO: We could chop up
  // target to fit.
  if (Target.OffsetInBits < CurrentFragment->OffsetInBits ||
      Target.SizeInBits > CurrentFragment->SizeInBits)
    return std::nullopt;

  // Target fits within the current fragment.
  return Target;
}
...
static void migrateDebugInfo(AllocaInst *OldAlloca, bool IsSplit,
  ...
  LLVM_DEBUG(dbgs() << "    AbsoluteOffsetInBits: " << AbsoluteOffsetInBits
                    << "\n");
  ...
  for (DbgAssignIntrinsic *DbgAssign : MarkerRange) {
    ...
    if (IsSplit) {
      ...
      std::optional<DIExpression::FragmentInfo> NewFragment = calculateFragment(
          AbsoluteOffsetInBits, SliceSizeInBits, BaseFragment, CurrentFragment);
