...
static BinaryOperator *LowerNegateToMultiply(Instruction *Neg) {
  ...
  BinaryOperator *Res = CreateMul(Neg->getOperand(OpNo), NegOne, "", Neg->getIterator(), Neg);
...
static BinaryOperator *convertOrWithNoCommonBitsToAdd(Instruction *Or) {
  ...
  BinaryOperator *New =
      CreateAdd(Or->getOperand(0), Or->getOperand(1), "", Or->getIterator(), Or);
...
static BinaryOperator *BreakUpSubtract(Instruction *Sub,
  ...
  BinaryOperator *New = CreateAdd(Sub->getOperand(0), NegVal, "", Sub->getIterator(), Sub);
...
static BinaryOperator *ConvertShiftToMul(Instruction *Shl) {
  ...
  BinaryOperator *Mul =
    BinaryOperator::CreateMul(Shl->getOperand(0), MulCst, "", Shl->getIterator());
...
Value *ReassociatePass::OptimizeXor(Instruction *I,
  ...
  for (unsigned i = 0, e = Opnds.size(); i < e; i++) {
    ...
    if (!ConstOpnd.isZero() && CombineXorOpnd(I->getIterator(), CurrOpnd, ConstOpnd, CV)) {
      Changed = true;
      if (CV)
        *CurrOpnd = XorOpnd(CV);
      else {
