...
bool BCECmpBlock::doesOtherWork() const {
  ...
  for (const Instruction &Inst : *BB) {
    // jmorse: there's a noteworthy problem here: we split the blocks apart when we
    // see debug-info intrinsics, which is wrong. But by chance, the re-formed
    // blocks happen to be identical to the non-debug-info case. In RemoveDIs
    // mode this doesn't happen (yet), so RemoveDIs mode drops debug-info where
    // dbg.value mode does not. This is a scenario where we require more special
    // casing / handling of debug-info to preserve it in RemoveDIs mode, but
