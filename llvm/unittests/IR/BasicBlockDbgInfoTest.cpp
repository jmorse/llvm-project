...
TEST(BasicBlockDbgInfoTest, MarkerOperations) {
  ...
  DPMarker *EndMarker = &BB.TrailingDPValues;
  ...
  EXPECT_EQ(BB.getNextMarker(Instr2), EndMarker);

  // There should be two DPValues,
  EXPECT_EQ(Marker1->StoredDPValues.size(), 1);
  EXPECT_EQ(Marker2->StoredDPValues.size(), 1);
  ...
  EXPECT_EQ(BB.size(), 1);
  EXPECT_EQ(Marker2->StoredDPValues.size(), 2);
  ...
  // the trailing marker.
  EXPECT_TRUE(BB.TrailingDPValues.StoredDPValues.empty());
  Instr2->removeFromParent();
  EXPECT_TRUE(BB.empty());
  EXPECT_EQ(BB.TrailingDPValues.StoredDPValues.size(), 2);
  // Again, these should arrive in the correct order.

  DPVs.clear();
  for (DPValue &DPV : BB.TrailingDPValues.getDbgValueRange())
    DPVs.push_back(&DPV);
  EXPECT_EQ(DPVs[0], DPV1);
  EXPECT_EQ(DPVs[1], DPV2);
  ...
  EXPECT_TRUE(BB.TrailingDPValues.StoredDPValues.empty());
  EXPECT_EQ(Instr1->DbgMarker->StoredDPValues.size(), 2);
  // And now they'll fall down again,
  Instr1->removeFromParent();
  EXPECT_EQ(BB.TrailingDPValues.StoredDPValues.size(), 2);
  ...
  EXPECT_EQ(Instr2->DbgMarker->StoredDPValues.size(), 2);
  EXPECT_TRUE(BB.TrailingDPValues.StoredDPValues.empty());
...
TEST(BasicBlockDbgInfoTest, InstrDbgAccess) {
  ...
  ASSERT_TRUE(CInst->DbgMarker);
  ASSERT_EQ(CInst->DbgMarker->StoredDPValues.size(), 1);
  DPValue *DPV1 = &*CInst->DbgMarker->StoredDPValues.begin();
  ASSERT_TRUE(DPV1);
  EXPECT_EQ(BInst->DbgMarker->StoredDPValues.size(), 0);
  ...
  EXPECT_EQ(BInst->DbgMarker->StoredDPValues.size(), 1);
  DPValue *DPV2 = &*BInst->DbgMarker->StoredDPValues.begin();
  EXPECT_EQ(std::distance(Range1.begin(), Range1.end()), 1);
  ...
  EXPECT_EQ(CInst->DbgMarker->StoredDPValues.size(), 0);
