...
public:
  ...
  bool vlocJoin(MachineBasicBlock &MBB, InstrRefBasedLDV::LiveIdxT &VLOCOutLocs,
    ...
    return LDV->vlocJoin(MBB, VLOCOutLocs, AllVars,
...
TEST_F(InstrRefLDVTest, MLocSingleBlock) {
  ...
  // machine location.
  TransferFunc[0].insert({0, ValueIDNum(0, 1, RspLoc)});
  initValueArray(InLocsPtr, 1, 2);
  initValueArray(OutLocsPtr, 1, 2);
  buildMLocValueMap(InLocsPtr, OutLocsPtr, TransferFunc);
  EXPECT_EQ(InLocs[0], ValueIDNum(0, 0, RspLoc));
...
TEST_F(InstrRefLDVTest, MLocNestedLoop) {
  ...
  // However: if we erase the inner-loop def, then we just have defs in the
...
TEST_F(InstrRefLDVTest, vlocJoinDiamond) {
  ...
  bool Result = vlocJoin(*MBB3, VLiveOutIdx, AllVars,
  ...
  Result = vlocJoin(*MBB3, VLiveOutIdx, AllVars,
                    AllBlocks, AllBlocks, JoinedLocs);
  EXPECT_FALSE(Result);
  JoinedLocs.clear();
...
TEST_F(InstrRefLDVTest, vlocJoinLoops) {
  ...
  bool Result = vlocJoin(*MBB1, VLiveOutIdx, AllVars,
                    AllBlocks, AllBlocks, JoinedLocs);
  EXPECT_TRUE(Result);
  auto It = JoinedLocs.find(Var);
  EXPECT_EQ(It->second.Kind, DbgValue::Def);
  ...
  Result = vlocJoin(*MBB1, VLiveOutIdx, AllVars,
                    AllBlocks, AllBlocks, JoinedLocs);
  EXPECT_FALSE(Result);
  It = JoinedLocs.find(Var);
  EXPECT_EQ(It->second.Kind, DbgValue::VPHI);
...
TEST_F(InstrRefLDVTest, VLocDiamondBlocks) {
  ...
  LocIdx RaxLoc = MTracker->lookupOrTrackRegister(RAX);
  ValueIDNum LiveInRsp = ValueIDNum(0, 0, RspLoc);
  ValueIDNum LiveInRax = ValueIDNum(0, 0, RaxLoc);
  ValueIDNum RspPHIInBlk3 = ValueIDNum(3, 0, RspLoc);
...
TEST_F(InstrRefLDVTest, VLocSimpleLoop) {
  ...
  ValueIDNum LiveInRsp = ValueIDNum(0, 0, RspLoc);
  ValueIDNum LiveInRax = ValueIDNum(0, 0, RaxLoc);
  ValueIDNum RspPHIInBlk1 = ValueIDNum(1, 0, RspLoc);
  ValueIDNum RspDefInBlk1 = ValueIDNum(1, 1, RspLoc);
  ValueIDNum RaxPHIInBlk1 = ValueIDNum(1, 0, RaxLoc);
...
TEST_F(InstrRefLDVTest, VLocNestedLoop) {
  ...
  ValueIDNum RspDefInBlk1 = ValueIDNum(1, 1, RspLoc);
  ValueIDNum RaxPHIInBlk1 = ValueIDNum(1, 0, RaxLoc);
  ValueIDNum RspPHIInBlk2 = ValueIDNum(2, 0, RspLoc);
  ValueIDNum RspDefInBlk2 = ValueIDNum(2, 1, RspLoc);
