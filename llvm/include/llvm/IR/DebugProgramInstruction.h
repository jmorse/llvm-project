...
public:
  ...
  simple_ilist<DPValue>::iterator getFirstDbgValue();
  ...
  simple_ilist<DPValue>::iterator cloneDbgInfoFrom(DPMarker *From, bool InsertAtHead, std::optional<simple_ilist<DPValue>::iterator> from_here);
  void dropDPValues();
  // To emulate the per-instruction erasure that istriviallydead... does.
  void dropOneDPValue(DPValue *DPV);
  iterator_range<simple_ilist<DPValue>::iterator> getRangeToEnd(simple_ilist<DPValue>::iterator);
};
