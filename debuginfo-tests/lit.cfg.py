...
# find the substitution for '%python' to use to invoke Dexter.
python_subst = next(subst for (key, subst) in
        llvm_config.config.substitutions if key == '%python')

tools = [
    ToolSubst('%test_debuginfo', command=os.path.join(
        config.debuginfo_tests_src_root, 'llgdb-tests', 'test_debuginfo.pl')),
    ToolSubst('%dexter', command='{} {}'.format(python_subst, os.path.join(
        config.debuginfo_tests_src_root, 'dexter', 'dexter.py'))),
]
